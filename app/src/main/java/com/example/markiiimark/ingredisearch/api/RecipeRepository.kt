...
import com.example.markiiimark.ingredisearch.model.RecipesContainer
...
class RecipeRepository(private val sharedPreferences: SharedPreferences) {
    ...
    fun addFavorite(item: Recipe) {
        val favorites = getFavoriteRecipies() + item
    ...
    fun getFavoriteRecipies(): List<Recipe> {
        val favoritesString = sharedPreferences.getString(FAVORITES_KEY, null)
        if (favoritesString != null) return gson.fromJson(favoritesString)
        return emptyList()
    }
    ...
    private fun markFavorites(recipesContainer: RecipesContainer?) {
        if (recipesContainer != null) {
            val favoriteRecipes = getFavoriteRecipies()
            if (favoriteRecipes.isNotEmpty()) {
                recipesContainer.recipes.forEach { item ->
    ...
    companion object {
        fun getRepository(context: Context) : RecipeRepository =
                RecipeRepository(context.getSharedPreferences("Favorites", Context.MODE_PRIVATE))
    }
